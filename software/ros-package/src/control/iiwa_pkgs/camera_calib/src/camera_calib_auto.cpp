...
string outdir;
std::vector<std::vector<double>> calib_poses_vector;
bool running = true;
...
void Callback_point(const sensor_msgs::PointCloud2::ConstPtr& msg)
{
    ...
    for(int cc = 0; cc < cloud->size(); cc++){
        uint32_t rgb = *reinterpret_cast<int*>(&cloud->points[cc].rgb);
...
void loadCalibPoses(int binId)
{
    ...
    system(("rosparam load " +resources_path + "/config/calib_poses.yaml").c_str());
// read bin pose
    for(int i = 0; i < 16; i ++){
        std::vector<double> calib_poses;
        nh.getParam("bin_2/position_"+std::to_string(i), calib_poses);  
        calib_poses_vector.push_back(calib_poses);
    }
