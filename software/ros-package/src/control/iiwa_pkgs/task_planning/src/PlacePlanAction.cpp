...
class PlacePlanAction
{
public:
    
  PlacePlanAction(std::string name) : 
    as_(nh_, name, false),
    action_name_(name)
  {
    ...
    #ifdef USE_ACTIONLIB
    as_.registerGoalCallback(boost::bind(&PlacePlanAction::goalCB, this));
    as_.start();
    #endif

    //as_.registerPreemptCallback(boost::bind(&PlacePlanAction::preemptCB, this));
    // #ifdef USE_SERVICE
    // ros::ServiceServer service = nh_.advertiseService("placeplan", &PlacePlanAction::goalCB, this);
    // #endif
...
protected:
  ...
  ros::NodeHandle nh_;
  actionlib::SimpleActionServer<task_planning::PlacePlanAction> as_;
  ...
  int data_count_;
  float sum_, sum_sq_;
  task_planning::PlacePlanFeedback feedback_;
  task_planning::PlacePlanResult result_;
  task_planning::PlacePlanGoal goal_;
  ros::Subscriber sub_;
...
void PlacePlanAction::convert3dUnOrganized(cv::Mat &objDepth, Eigen::Matrix3f &camIntrinsic, pcl::PointCloud<pcl::PointXYZ>::Ptr objCloud){
...
void PlacePlanAction::readDepthImage(cv::Mat &depthImg, std::string path){
...
void PlacePlanAction::toTransformationMatrix(Eigen::Matrix4f& camPose, std::vector<double> camPose7D){
...
void PlacePlanAction::InverseTransformationMatrix(Eigen::Matrix4f &origin, Eigen::Matrix4f &inverse){
...
Eigen::Matrix4f PlacePlanAction::transformPclFromWorldToTarget(pcl::PointCloud<pcl::PointXYZ>::Ptr target_pcl, geometry_msgs::Pose target_pose){
...
bool PlacePlanAction::checkRegionCollision(geometry_msgs::Pose center, double x_length, double y_length, double z_length){
...
geometry_msgs::Pose PlacePlanAction::getRetractPose(geometry_msgs::Pose target_pose, double x_length, double y_length, double z_length){
  int max_step = 9;
  int step_index = 0;
...
#ifdef USE_ACTIONLIB
void PlacePlanAction::goalCB()
#endif
#ifdef USE_SERVICE
bool PlacePlanAction::goalCB(task_planning::PlacePlan::Request  &req,
          task_planning::PlacePlan::Response &res)
#endif
{ 
  #ifdef USE_ACTIONLIB
    ...
    //publish_cube(target_object_pose_, x_length, y_length , z_length, cube_pub, 0, 0.9, 0.1);
...
void PlacePlanAction::initWorldPointCloud(){
...
void PlacePlanAction::publish_cube(geometry_msgs::Pose pose, double x, double y, double z, ros::Publisher pub ,double r, double g, double b)
{
...
void PlacePlanAction::publish_arrow(geometry_msgs::Point start, geometry_msgs::Point end)
{
...
{
  ros::init(argc, argv, "placeplan_action");
  ROS_WARN_STREAM("starting");
  ros::NodeHandle nh;
  PlacePlanAction placeplan_action(ros::this_node::getName());
      #ifdef USE_SERVICE
    ros::ServiceServer service = nh.advertiseService("/placeplan_service", &PlacePlanAction::goalCB, &placeplan_action);
    #endif
