...
#include "youbot_grasp/grasp_grasps.h"
...
plannedGrasp::plannedGrasp(cartesianGraspDirection in, double palm_angle)
{
  set_graspDirection(in);
  palm_rotation = palm_angle;
  palm_fallback = 0.025;
 // myPreshape = PR_None;
  myQuality = -1;
}

plannedGrasp::plannedGrasp(cartesianGraspDirection in, double palm_angle, double fallback)
{
  set_graspDirection(in);
  palm_rotation = palm_angle;
  palm_fallback = fallback;
 // myPreshape = PR_None;
  myQuality = -1;
}

plannedGrasp::plannedGrasp(cartesianGraspDirection in, double palm_angle, double fallback, geometry_msgs::Pose gripper_pose, geometry_msgs::Pose pre_gripper_pose)
{
  set_graspDirection(in);
  palm_rotation = palm_angle;
  palm_fallback = fallback;
 // myPreshape = PR_None;
  ...
  pre_gripper_pose_ = pre_gripper_pose;
}
...
*/
void           
plannedGrasp::set_fixedFingerDirection(tf::Vector3 in){
...
void 
plannedGrasp::set_grasp_msg(youbot_grasp::PlannedGrasp &grasp_msg)
{
  grasp_msg.Quality.data = myQuality;
  grasp_msg.grasp_point.x = myGraspDir.get_point().x();
  grasp_msg.grasp_point.y = myGraspDir.get_point().y();
  grasp_msg.grasp_point.z = myGraspDir.get_point().z();
  grasp_msg.grasp_dir.x = myGraspDir.get_dir().x();
  grasp_msg.grasp_dir.y = myGraspDir.get_dir().y();
  grasp_msg.grasp_dir.z = myGraspDir.get_dir().z();
  grasp_msg.palm_rotation.data = palm_rotation;
  grasp_msg.palm_fallback.data = palm_fallback;
  grasp_msg.gripper_pose = gripper_pose_;
  grasp_msg.pre_gripper_pose = pre_gripper_pose_;
}

void plannedGrasp::set_grasp_pose(geometry_msgs::Pose pose){
  gripper_pose_ = pose;
}

void plannedGrasp::set_pre_grasp_pose(geometry_msgs::Pose pose){
  pre_gripper_pose_ = pose;
}
// /*!
