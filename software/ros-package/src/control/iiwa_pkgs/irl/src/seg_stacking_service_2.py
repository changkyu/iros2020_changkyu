...
class ObjectMessageServer:
	def __init__(self):
		rospy.init_node("object_message_server_2")
		print "Initializing the segmentation service object_message_server_2"
		self.service = rospy.Service("/iiwa/object_message_2",SegmentationCall,self.call_service)

		self.threshold = 20000 
		self.table_height = -0.25	

		rospy.wait_for_service("segmentation_2/lccp_2dseg")
		self.segmentation_service = rospy.ServiceProxy("segmentation_2/lccp_2dseg",seg_scene_srv)
		print "Published the service..."
		self.camera_RT = np.loadtxt(cam_info_path+"/input/camera_2_calibration_data/cam_info.txt")
		
		self.surfaces = []
		self.labels = list(string.ascii_uppercase)

		self.workspace = [(0.39, 0.34, -0.25), 0.1]
	...
	def check_if_in_roi(self,i,j):
		...
		for i,coord in enumerate(center):
			if abs(coord - self.workspace[0][i]) > self.workspace[1] :
