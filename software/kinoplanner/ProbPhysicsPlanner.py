...
def error2prob(error):
    return np.exp(-error)

class ProbPhysicsPlanner:

    def __init__(self, name, img, models, meter_per_pixel,
                       setup=None, 
                       contact_resolution=0.01,
                       pushing_dists=[0.20,0.15,0.10,0.05,0.03,0.02,0.01] ):
        ...
        self.estimators = []
        for model in models:
            error_cell = model['error_cell']
            cellinfo = model['cellinfo']
            probs.append(error2prob(error_cell))
            cellinfos.append(cellinfo)

        self.probs_obs = []
        probs_sum = sum(probs)
        self.probs_models = [prob/float(probs_sum) for prob in probs]
        self.planner = KinoPlanner(setup=setup,
                                   debug=False, use_multiproc=True,
                                   contact_resolution=contact_resolution, 
                                   pushing_dists=pushing_dists)
        self.planner.addObject(name,img,cellinfos,meter_per_pixel,probs=self.probs_models)
    ...
    def addObservation(self, pos_begin, pos_final, pos_push, vec_push, duration):
        ...
        action = {'pos':pos,'yaw':0,'velocity':velocity,'duration':duration}
        trans = planner.computeTransition_models(self.name, action)

        cellpos0 = planner.getCellPos(self.name+'_0',0,0,0)
        probs = []
        for i, tran in enumerate(trans):
            cellpos1 = planner.getCellPos(self.name+'_0',x_res,y_res,yaw_res)
    ...
    def selectGoals(self, obj, goals):
        return self.planner.selectGoals(obj, goals)

    def plan(self, init, goal, pred_step=1, thres=0.02, n_plans=10, actions_rejected=[]):
        
        waypoints = self.planner.plan_rrt(self.name+'_0', init, goal)
        plans = self.planner.plan_prop(self.name, waypoints, horizon=pred_step, thres=thres, n_plans=n_plans, actions_rejected=actions_rejected)

        # plans are sorted by distances to waypoint
        # re-order by succ -> failure
        plans_succ = []
        plans_fail = []
        for plan in plans:
            if plan['succ'] > 1.00:
                plans_succ.append({'nexts':plan['nexts'], 'action':plan['action']})
            else:
                plans_fail.append({'nexts':plan['nexts'], 'action':plan['action']})
        return plans_succ + plans_fail
